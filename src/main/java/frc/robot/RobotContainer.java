...
public class RobotContainer {
  ...
  public final ClimbUp m_climbUp = new ClimbUp(m_climber);
  ...
  private void configureBindings() {
    ...
    m_JoystickLeft.button(1).whileTrue(m_shoot);
    ...
    m_JoystickLeft.button(2).onTrue(Commands.runOnce(()->m_transit.armDown())).whileTrue(m_upLift).onFalse(Commands.runOnce(()->m_transit.armUp()));
    ...
    m_JoystickLeft.povUp().or(m_JoystickLeft.povUpLeft()).or(m_JoystickLeft.povUpRight()).and(()->RobotContainer.m_driveMode!=Constants.DriveMode.TANKDRIVE).whileTrue(m_spoutOnce);
    m_JoystickLeft.povDown().or(m_JoystickLeft.povDownLeft()).or(m_JoystickLeft.povDownRight()).and(()->RobotContainer.m_driveMode!=Constants.DriveMode.TANKDRIVE).whileTrue(m_detrudeOnce);
    m_JoystickLeft.povRight().and(()->RobotContainer.m_driveMode==Constants.DriveMode.CLIMBUP).onTrue(Commands.runOnce(()->m_climbUp.setAsync(true)));
    m_JoystickLeft.povLeft().and(()->RobotContainer.m_driveMode==Constants.DriveMode.CLIMBUP).onTrue(Commands.runOnce(()->m_climbUp.setAsync(false)));
  ...
  public Command getAutonomousCommand() {
    ...
    if(m_autoMode!=AutoMode.AP){
      return Autos.follow_trajectory(m_autoMode, m_driveTrain, m_transit);
    }else{
