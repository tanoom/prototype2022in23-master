...
public class SwerveDrive extends CommandBase {
  ...
  public void execute() {
    if(RobotContainer.m_driveMode==Constants.DriveMode.SWERVEDRIVE||(RobotContainer.m_driveMode==Constants.DriveMode.CLIMBUP&&!ClimbUp.async_control)){
      final var xSpeed =
          -m_xspeedLimiter.calculate(MathUtil.applyDeadband(RobotContainer.m_JoystickRight.getY(), 0.02))
              * DriveTrain.MaxVelocity;
      final var ySpeed =
          -m_yspeedLimiter.calculate(MathUtil.applyDeadband(RobotContainer.m_JoystickRight.getX(), 0.02))
              * DriveTrain.MaxVelocity;
      final var rot =
          -m_rotLimiter.calculate(MathUtil.applyDeadband(RobotContainer.m_JoystickRight.getZ(), 0.2))
              * 2* Math.PI;
